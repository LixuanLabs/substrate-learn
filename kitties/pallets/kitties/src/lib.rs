...
/// kitties的存储结构：
/// 1. Kitty { dna, price, gender, owner }
/// 2. Kitties { hash: Kitty }
/// 3. KittiesOwned { owner: [ hash ] } , 一个用户可以有多个kitty
/// 
...
pub mod pallet {
	...
	pub trait Config: frame_system::Config {
		...
		#[pallet::constant]
		type PledgeCount: Get<BalanceOf<Self>>;
	}
	...
	pub type KittyCnt<T> = StorageValue<_, u64, ValueQuery>;
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn create_kitty(origin: OriginFor<T>) -> DispatchResult {
			...
			// 质押fee
			ensure!(T::Currency::free_balance(&sender) == T::PledgeCount::get(), Error::<T>::NotEnoughBalance);
			// T::Currency::reserve(&sender, T::PledgeCount::get())?;
			Self::deposit_event(Event::Created(sender, kitty_id));
			Ok(())
		}
		#[pallet::weight(0)]
		pub fn transfer(origin: OriginFor<T>, kitty_id: T::Hash, to: T::AccountId) -> DispatchResult {
		...
		pub fn buy_kitty(origin: OriginFor<T>, kitty_id: T::Hash, bid_price: BalanceOf<T>) -> DispatchResult {
			...
			T::Currency::transfer(&seller, &buyer, bid_price, ExistenceRequirement::KeepAlive)?;
		...
		#[pallet::weight(100)]
		pub fn sell_kitty(origin: OriginFor<T>, kitty_id: T::Hash, price: Option<BalanceOf<T>>) -> DispatchResult {
			let sender = ensure_signed(origin)?;
