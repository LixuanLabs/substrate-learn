...
pub mod pallet {
	...
	const HTTP_REMOTE_PRICE: &str = "https://api.coincap.io/v2/assets/polkadot";
	...
	struct PolkPriceInfoSer {
		data: PolkPriceInfo
	}

	#[derive(Debug, Deserialize, Encode, Decode, Default)]
	struct PolkPriceInfo {
		// Specify our own deserializing function to convert JSON string to vector of bytes
		#[serde(deserialize_with = "de_string_to_str")]
		priceUsd: (u64, Permill),
	}
	...
	pub fn de_string_to_str<'de, D>(de: D) -> Result<(u64, Permill), D::Error>
	where
	D: Deserializer<'de>,
	{
		let s: &str = Deserialize::deserialize(de)?;
		let split_vec: Vec<&str> = s.split(".").collect();
		log::info!("split_vec[1] {:?}", split_vec[1]);
		Ok((split_vec[0].parse::<u64>().unwrap_or(0), Permill::from_parts(split_vec[1][0..5].parse::<u32>().unwrap_or(0))))
	}
	...
	pub enum Event<T: Config> {
		...
		SavePrice(Option<T::AccountId>, (u64, Permill))
	}
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn submit_price_unsigned(origin: OriginFor<T>, price: (u64, Permill)) -> DispatchResult {
			...
			Self::deposit_event(Event::SavePrice(None, price));
			Ok(())
		}
	...
	impl<T: Config> Pallet<T> {
		...
		fn fetch_price_info() -> Result<(), Error<T>> {
			...
			match Self::fetch_n_parse_price(HTTP_REMOTE_PRICE) {
				Ok(p_info) => {
					// p_info
					log::info!("priceUsd {:?}", p_info.priceUsd);
					let call = Call::submit_price_unsigned(p_info.priceUsd);

					// `submit_unsigned_transaction` returns a type of `Result<(), ()>`
					//   ref: https://substrate.dev/rustdocs/v2.0.0/frame_system/offchain/struct.SubmitTransaction.html#method.submit_unsigned_transaction
					SubmitTransaction::<T, Call<T>>::submit_unsigned_transaction(call.into())
					.map_err(|_| {
						log::error!("Failed in submit_price_unsigned");
		...
		/// Fetch from remote and deserialize the JSON to a struct
		fn fetch_n_parse_price(url: &str) -> Result<PolkPriceInfo, Error<T>> {
			let resp_bytes = Self::fetch_from_remote_common(url).map_err(|e| {
				log::error!("fetch_from_remote error: {:?}", e);
				<Error<T>>::HttpFetchingError
			})?;
		...
		fn fetch_from_remote_common(url: &str) -> Result<Vec<u8>, Error<T>> {
